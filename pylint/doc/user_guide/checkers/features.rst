::resource-leak-maybe (R1738): *Resource '%s' opened without a context manager and never explicitly closed*
  Emitted when a known resource allocation is neither protected by a context manager nor explicitly released,
  which increases the risk of resource leaks.
Pylint features
===============
...
:optional-import-error (W0405): *Optional dependency %s could not be imported*
  Emitted when an import fails but the module was configured through
  ``optional-import-modules`` as an optional dependency.
...
:high-risk-todo (W0512):
  Used when a warning note matches the configured ``high-risk-fixme-rgx`` pattern indicating
  the TODO/FIXME is critical.
