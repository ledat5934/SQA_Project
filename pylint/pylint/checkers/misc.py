    ...
    * encoding issues.
    ...
    msgs = {
        ...
        "W0512": (
            "%s",
            "high-risk-todo",
            "Used when a TODO/FIXME style note matches the configured high risk pattern.",
            {"default_enabled": False},
        ),
    }
    ...
    options = (
        ...
        (
            "high-risk-fixme-rgx",
            {
                ...
                "default": "",
                "help": (
                    "Regular expression used to flag fixme notes as high risk. "
                    "When provided, matching notes emit the 'high-risk-todo' message."
                ),
            },
        ),
    )
    ...
    def open(self) -> None:
        ...
        if self.linter.config.high_risk_fixme_rgx:
            self._high_risk_fixme_pattern = re.compile(
                self.linter.config.high_risk_fixme_rgx, re.I
            )
        else:
            self._high_risk_fixme_pattern = None
    ...
    def _report_fixme(self, msg: str, line: int, col_offset: int) -> None:
        message_id = "fixme"
        if self._high_risk_fixme_pattern and self._high_risk_fixme_pattern.search(msg):
            message_id = "high-risk-todo"
        self.add_message(message_id, line=line, col_offset=col_offset, args=msg)
