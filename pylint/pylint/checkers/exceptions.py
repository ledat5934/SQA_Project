...
def _is_logging_debug_call(node: nodes.NodeNG) -> bool:
    """Return True if the node represents a logging debug call."""
    if not isinstance(node, nodes.Call):
        return False
    if not isinstance(node.func, nodes.Attribute):
        return False
    if node.func.attrname != "debug":
        return False
    expr = node.func.expr
    if isinstance(expr, nodes.Name):
        return expr.name in {"logging", "logger", "log"}
    if isinstance(expr, nodes.Attribute):
        return expr.attrname in {"logger", "log"}
    return False


def _has_trivial_handler_body(body: list[nodes.NodeNG]) -> bool:
    """Return True if every statement in the body is effectively a no-op."""
    if not body:
        return True
    for stmt in body:
        ...
        if isinstance(stmt, nodes.Expr):
            value = stmt.value
            if isinstance(value, nodes.Const) and value.value in (None, Ellipsis):
                continue
            if _is_logging_debug_call(value):
                continue
        return False
    return True


def _format_exception_names(names: list[str], is_bare: bool) -> str:
    if is_bare or not names:
        return "bare-except"
...
MSGS: dict[str, MessageDefinitionTuple] = {
    ...
    "W0708": (
        "Swallowed exception for %s; handler only performs no-op statements",
        "swallowed-exception",
        "Used when a broad exception handler (bare or configured as overgeneral) "
        "only contains no-op operations such as ``pass`` or ``logging.debug``. "
        "This usually hides real errors.",
    ),
    "W0709": (
        "Empty exception handler for %s",
        "empty-except-body",
        "Used when an except block does not execute any meaningful statements. "
        "An empty handler makes debugging harder and should either act on the exception "
        "or re-raise it.",
    ),
    "W0718": (
...
class ExceptionsChecker(checkers.BaseChecker):
    ...
    def visit_try(self, node: nodes.Try) -> None:
        ...
        for index, handler in enumerate(node.handlers):
            handler_is_trivial = _has_trivial_handler_body(handler.body)
            ...
            if handler.type is None:
                ...
                self._maybe_report_trivial_handler(
                    handler,
                    handled_names,
                    handler_is_trivial,
                    catches_broad,
                ...
                try:
                    ...
                    self._maybe_report_trivial_handler(
                        handler,
                        handled_names or [handler.type.as_string()],
                        handler_is_trivial,
                        catches_broad,
                    )
                    continue
                ...
                for part, exception in exceptions:
                    ...
                    handled_names.append(exception.name)
                    ...
                    if self._is_overgeneral_exception(exception):
                        catches_broad = True
                        if not _is_raising(handler.body):
                            self.add_message(
                                "broad-exception-caught",
    ...
    def _maybe_report_trivial_handler(
        self,
        handler: nodes.ExceptHandler,
        handled_names: list[str],
        handler_is_trivial: bool,
        catches_broad: bool,
    ) -> None:
        if not handler_is_trivial:
            return
        description = _format_exception_names(handled_names, handler.type is None)
        self.add_message(
            "empty-except-body",
            node=handler,
            args=(description,),
