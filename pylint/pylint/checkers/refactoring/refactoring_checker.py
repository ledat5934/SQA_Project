...
class ConsiderUsingWithStack(NamedTuple):
    ...
    module_scope: dict[str, ResourceCandidate] = {}
    class_scope: dict[str, ResourceCandidate] = {}
    function_scope: dict[str, ResourceCandidate] = {}

    def __iter__(self) -> Iterator[dict[str, ResourceCandidate]]:
...
class RefactoringChecker(checkers.BaseTokenChecker):
    ...
    msgs = {
        ...
        "R1738": (
            "Resource '%s' opened without a context manager and never explicitly closed",
            "resource-leak-maybe",
            "Emitted when a known resource allocation is neither protected by a context manager "
            "nor explicitly released, which increases the risk of resource leaks.",
        ),
        "R1733": (
    ...
    @utils.only_required_for_messages("consider-using-with", "resource-leak-maybe")
    ...
    @utils.only_required_for_messages(
        "redefined-argument-from-local", "consider-using-with", "resource-leak-maybe"
    )
    ...
    def visit_call(self, node: nodes.Call) -> None:
        ...
        self._mark_resource_released(node)
    ...
    def _emit_consider_using_with_if_needed(
        self, stack: dict[str, ResourceCandidate]
    ) -> None:
        for name, candidate in stack.items():
            self.add_message("consider-using-with", node=candidate.node)
    ...
    def _check_consider_using_with(self, node: nodes.Call) -> None:
        if _is_inside_context_manager(node) or _is_a_return_statement(node):
            ...
            return
        stack = self._consider_using_with_stack.get_stack_for_frame(node.frame())
        if any(candidate.node == node for candidate in stack.values()):
    ...
    def _mark_resource_released(self, node: nodes.Call) -> None:
        if not isinstance(node.func, nodes.Attribute):
            return
        if node.func.attrname not in RESOURCE_RELEASE_METHODS:
            return
        expr = node.func.expr
        if not isinstance(expr, nodes.Name):
            return
        for stack in self._consider_using_with_stack:
            candidate = stack.get(expr.name)
            if candidate:
