...
class BasicChecker(_BasicChecker):
    ...
    * uses of the global statement
    ...
    def _check_dangerous_default(self, node: nodes.FunctionDef) -> None:
        ...
        dangerous_defaults = self._collect_default_candidates(node, is_iterable)
        if not dangerous_defaults:
            return
        mutated_params = self._collect_mutated_parameters(node)
        for arg_name, (msg, require_mutation) in dangerous_defaults.items():
            if require_mutation and arg_name not in mutated_params:
                continue
    ...
    def _collect_default_candidates(
        ...
        is_iterable: "Callable[[nodes.NodeNG], bool]",
    ) -> dict[str, tuple[str, bool]]:
        candidates: dict[str, tuple[str, bool]] = {}
        positional = node.args.posonlyargs + node.args.args
        positional_defaults = node.args.defaults or []
        if positional_defaults:
            relevant = positional[-len(positional_defaults) :]
            for arg, default in zip(relevant, positional_defaults):
                info = self._describe_default(default, is_iterable)
                if info:
                    candidates[arg.name] = info
    ...
    def _describe_default(
        ...
        if is_iterable(default):
            return default.as_string(), False

        return None

    def _collect_mutated_parameters(self, node: nodes.FunctionDef) -> set[str]:
        param_names = {
            arg.name
            for arg in (
        ...
        def in_current_scope(stmt: nodes.NodeNG) -> bool:
            return stmt.scope() is node

        for call in node.nodes_of_class(nodes.Call):
            if not in_current_scope(call):
                continue
            if not isinstance(call.func, nodes.Attribute):
                continue
            owner = call.func.expr
            if isinstance(owner, nodes.Name) and owner.name in param_names:
                if call.func.attrname in MUTATING_METHODS:
                    mutated.add(owner.name)

        def track_target(target: nodes.NodeNG) -> None:
            name = self._subscripted_param_name(target, param_names)
            if name:
                mutated.add(name)

        for assign in node.nodes_of_class(nodes.Assign):
            if not in_current_scope(assign):
                continue
            for target in assign.targets:
                track_target(target)
        for aug in node.nodes_of_class(nodes.AugAssign):
            if not in_current_scope(aug):
                continue
            track_target(aug.target)
        for delete in node.nodes_of_class(nodes.Delete):
            if not in_current_scope(delete):
                continue
    ...
    @staticmethod
    def _subscripted_param_name(
        target: nodes.NodeNG, param_names: set[str]
    ) -> str | None:
        if isinstance(target, nodes.Subscript) and isinstance(target.value, nodes.Name):
            if target.value.name in param_names:
                return target.value.name
        if isinstance(target, nodes.Attribute) and isinstance(target.expr, nodes.Name):
            if target.expr.name in param_names:
